    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <ul class="nav nav-tabs">
                <li role="presentation" class="active"><a href="{{route('admin_subject')}}">List</a></li>
                <li role="presentation" ><a href="{{route('admin_subject_create')}}">Create</a></li>
            </ul>
            ...
            @endif
             ...
             <table class="table">
                 ...
                 <tbody>
                     @foreach($subjects as $morls)
                        <tr>
                            <td>{{$morls->subject_name}}</td>
                            <td>
                                @if($morls->status_id == 0)
                                    <button type="button" class="btn btn-danger btn-xs">Close</button>
                                @elseif($morls->status_id == 1)
                                    <button type="button" class="btn btn-success btn-xs">Opened</button>
                                @endif
                            </td>
                            <td>{{$morls->enroll_code}}</td>
                            <td>{{$morls->teachers_code}}</td>
                            <td>
                                <a href="{{route('admin_subject_view', ['id'=> $morls->id])}}" class="btn btn-info btn-xs">Create</a>
                                <a href="{{route('admin_subject_status', ['id'=> $morls->id])}}" class="btn btn-success btn-xs">Open</a>
                                <a href="{{route('view_questions_subject', ['subject'=> $morls->id])}}" class="btn btn-danger btn-xs">View Questions</a>
                            </td>
                        </tr>
                     @endforeach
                 </tbody>
